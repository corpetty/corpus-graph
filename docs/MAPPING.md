...
To run the **automated extraction runner** (`make extract` — corpus-graph#1), give each text source a `file` prop pointing to a file under the profile dir (e.g. `"file": "refs/rfc9111.txt"`), and optionally set top-level `"extractFrom": ["Reference"]` in `ontology.json` to the node types that are text sources (it defaults to the subject types of your evidence predicates). The runner sends the closed-world catalog + each source to an LLM and writes re-validated triples to `interpretive/<source>.jsonl`. Sources larger than `--chunk-tokens` are automatically chunked (heading-aware), extracted per chunk, and merged with `(s,p,o)` dedup keeping the highest-confidence row — resumable via a per-chunk cache.
